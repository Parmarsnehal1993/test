                                                ...
                                                @foreach($userDebts['data'] as $userDebtsKey => $userDebtsValue)
                                                    @php $totalDebtsAmount += $userDebtsValue['amountOwned'] @endphp
                                                        <ul class="table-body align-items-center flex-wrap debts_data" id="getDebtsData">
                                                            ...
                                                            @php 
                                                                $average = $userDebtsValue['amountOwned'] / $total_amount * 100;
                                                                $dangerClass = "";
                                                                $debts_percentage = number_format($average);
                                                                if($debts_percentage == 25 || $debts_percentage >= 25){
                                                                    $dangerClass = "text-danger";
                                                                }
                                                            @endphp
                                                            <li class="debts_percentage {{ $dangerClass }}" id="single_debts_percentage_value_{{ $userDebtsCounter }}">{{ $debts_percentage }}%</li>
            ...
            success:function(data)
            {
                ...
                $.ajax({
                    ...
                    success:function(response)
                    {
                        ...
                        $('#single_debts_percentage_value_'+update_debt_counter).html(response.total_percentage+'%');
                        var total_debts = 0;
