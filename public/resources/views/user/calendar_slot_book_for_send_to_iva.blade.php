@php
    ...
    //$currentTime = '15:30';
@endphp
<div id="slot_book_for_iva_advisor" class="modal fade slot_book"  tabindex="-1" role="dialog" aria-labelledby="slot_book">
    <div class="modal-dialog modal-sm">
        <div class="modal-content card card-secondary">
            <div class="modal-header">
                <div class="card-title text-center width-100">IVA Advisor Availability</div>
            ...
            <div class="modal-body">
                ...
                <!-- Single row from monday to sunday calendar schedule code start -->
                    <!-- here it will load all the iva advisor name which is available on the particular date and for particular time slot -->
                                                ...
                                                @foreach($allAgentDataForCalendarValue[$currentWeekDatesValue] as $agentKey => $agentValue)
                                                     @if(isset($agentValue['agent_id']) && !empty($agentValue['agent_id']))
                                                        @php
                                                            ...
                                                            $str .= '<div class="dropdown-item select_agent_for_iva_calendar" data-agent_id="'.$agentValue["agent_id"].'" data-time_slot="'.$agentValue["slot"].'" data-slot_date="'.$agentValue["date"].'" data-slot_time="'.$loopHours[0].'">'.$agentValue["agent_name"].'&nbsp;&nbsp;<span class="text-success">'.$agentValue["total_case"].'</span></div>';
                                                        @endphp
                                                    @endif
                                                @endforeach
                                                @if($agentCount == 0)
