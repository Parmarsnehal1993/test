...
<section class="main-content">
    <div class="container">
        ...
        <section class="card-details" style="padding: 55px 60px">
            <form action="{{ route('user.save_callback') }}" method="POST">
                @csrf
                <!-- hidden fields start -->
                <input type="hidden" id="userId" name="userId" value="{{ $userInfo->user_id }}">
                <input type="hidden" id="callbackDateTime" name="callbackDateTime">
                <!-- hidden fields end -->
                ...
                <div class="text-right">
                    <input type="submit" class="btn btn-primary"  style="margin: 0" title="Save Callback" value="Save Callback">
                </div>
            </form>
        </section>
        <!-- card details end -->
...
$(function () {
    $(".datepicker").datepicker({
        ...
        onSelect: function(dateText, inst) {
            var date = $(this).val();
            $("#callbackDateTime").val(date.toString());
        }
    });
});
</script>
