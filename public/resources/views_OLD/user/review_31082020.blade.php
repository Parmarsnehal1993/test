    ...
    @php
        ...
        if(isset($userListSignupQuestions['data'][0]['is_process']) && !empty($userListSignupQuestions['data'][0]['is_process']) && strtolower($userListSignupQuestions['data'][0]['is_process']) == 'automatic') {
            $IODResult = getTotalIncomeOutcomeDisposable($userInfo->user_id);
            if(isset($IODResult) && !empty($IODResult)) {
                $totalIncome = $IODResult['totalIncome'];
                $totalOutgoings = $IODResult['totalOutgoings'];
                $totalDisposable = $IODResult['totalDisposable'];
            }
    ...
    <div class="row">
        ...
        <div class="col-6">
            <div class="card-body">
                <h1 class="text-center line-height-auto height-auto">&#163;&nbsp;{{ number_format($totalDisposable) }}</h1>
            </div>
        </div>
    </div>
    @else
